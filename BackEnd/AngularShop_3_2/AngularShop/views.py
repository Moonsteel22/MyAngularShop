...
def registration(request):
    if(request.method == 'POST'):
        ...
        newUser=UserTable(login=data['login'],name=data['name'],surname=data['surname'],password=data['password'],
                          phone=data['phone'],age=data['age'],email=data['email'],imgurl='',role='U',registrationdate=datetime.now())
        newUser.save()
            ...
            if (UserTable.objects.filter(password=json.loads(request.body)['password'])):
                ...
                return JsonResponse([{'code': 2, 'message': 'Вы успешно вошли','data':
                                     {'Name':data.name,'Email':data.email,'Registdate':data.registrationdate,
                                      'login':data.login,'pk':data.pk,'role':data.role}
                                      }],safe=False)
            else:
...
def clearCart(request):
    if(request.method=='GET'):
        print()
        if(request.GET.get('userID',-1)==-1):
             return JsonResponse({'code':400,'message':'Wrong user'},safe=False)
        
        Cart.objects.filter(userid=request.GET.get('userID',-1)).delete()
        return JsonResponse({'code':200,'message':'Cart has been cleared'},safe=False)
    return JsonResponse({'code':400,'message':'Ошибка запроса'},safe=False)
